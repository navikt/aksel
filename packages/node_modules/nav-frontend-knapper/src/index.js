...
/**
 *  Knapp, Hovedknapp og Fareknapp arver alle fra KnappBase, og har da samme props. Unntaket er `type` som blir satt av de respektive underklassene.
 */
class KnappBase extends Component {
    render() {
        const { children, className, ...props } = this.props;

        const spinner = props.spinner ? <span className="knapp__spinner" /> : null;
        const disabled = skalVareDisabled(props);
        const domProps = omit({ ...props, disabled }, 'type', 'mini', 'spinner', 'autoDisableVedSpinner', 'inaktivKlasseVedDisabled');
