...
<Alertstripe type="advarsel">
    Av tilgjengelighetshensyn så <strong>skal</strong> alle inputfelter ha en tilhørende <InlineCode>label</InlineCode> (<Lenke href="https://www.w3.org/TR/WCAG21/#labels-or-instructions">WCAG 3.3.2</Lenke>).
</Alertstripe>
...
Alternativt kan du importere `Label`-komponenten og manuelt sette denne utenfor inputfeltet:

<Alertstripe type="advarsel">
    Husk å gi Label-komponenten korrekt <InlineCode>for</InlineCode>-attributt (som skal ha samme verdi som inputfeltets <InlineCode>id</InlineCode>-attributt) slik at disse blir sammenkoblet riktig.
</Alertstripe>

<Example>
    <Label for="min-input">Mitt skjemafelt:</Label>
    <Input id="min-input" />
</Example>
...
<Example>
    <div style={{'display':'flex', 'alignItems': 'center'}}>
        <Label for="min-input-2" style={{'margin':0, 'marginRight':'1rem'}}>Mitt skjemafelt:</Label>
...
Unntaksvis kan man også angi `label` via inputfeltets `aria-label`-attributt. Dette er foreløpig kun tillatt på inputfelter inne i tabell-rader eller lignende tilfeller hvor det vil være unaturlig/umulig å ha en synlig tekst-label.

<Example>
    <Input aria-label="Mitt skjemafelt:" />
</Example>
...
<Example>
    <SkjemaGruppe>
        <label className="skjemaelement__label" for="min-input">
            <Element>Inputfelt-label</Element>
        </label>
        <Normaltekst>Innholdet vil vises mellom label og inputfelt.</Normaltekst>
        <Input id="min-input" />
    </SkjemaGruppe>
</Example>
...
<Example>
    <Input label="Inputfelt-label" feil={{feilmelding: 'Her er det noe feil'}} />
</Example>
...
<Example>
    <SkjemaGruppe feil={{feilmelding: 'Her er det noe feil'}}>
