...
export const getHtmlCodeForComponent = (
  component,
  activeModifiers,
  children
) => {
...
export const getReactCodeForComponent = (
  component,
  activeModifiers,
  children
) => {
  const jsxMainComponent = renderComponentWithModifiersAndChildren(
    component,
    activeModifiers,
    children
  );
...
const getMatchingCSSRulesForElement = (
  el,
  css = el.ownerDocument.styleSheets
) => {
  ...
  return rules.filter(
    (rule) => rule && rule.selectorText && el.matches(rule.selectorText)
  );
};
...
const getMatchingCSSRulesForElementWithChildren = (el) => {
  ...
  return allElements
    .map((element) => [
      ...getMatchingCSSRulesForElement(element, stylesheetRoot),
    ])
