...
function EkspanderbartpanelPure(props) {
    ...
    const heading = <Typografi {...headingConfig}>{tittel}</Typografi>;
    const content = apen ? (<div className="ekspanderbartPanel__innhold">{children}</div>) : null;

    return (
        <div className={cls(className, props)} {...renderProps}>
            <button className="ekspanderbartPanel__hode" onClick={onClick}>
                {heading}
