...
class MobileNav extends React.Component {
  ...
  renderRoute = (route, index) => {
    const open =
      this.props.history.location.pathname.indexOf(route.path) !== -1 &&
      route.routes;
    return (
        ...
        {route.routes && (
          <ul>
            {route.routes
              ...
              .map((filteredRoute, i) => this.renderRoute(filteredRoute, i))}
          </ul>
        )}
      </MobileNavMenuItem>
    );
        ...
        <nav className="mobile-nav__drawer" aria-label="main mobile">
          ...
          <ul className="nav-list">
            {routeConfig
              ...
              .map((route, index) => this.renderRoute(route, index))}
            <li>
              <a
