...
| **Component props API tool**          | Tool            | **Medium** | ✅ **COMPLETED** — Fetches curated props from `komponent_artikkel` → `props_seksjon` → `ds_props` in Sanity. Website exposes `/api/component-props?slug=` endpoint returning structured JSON per component part. Filters `@private` props, resolves enum types, appends `as` for overridable components, sorts deprecated props last. |
...
5. ✅ Component props API explorer **[COMPLETED]**
