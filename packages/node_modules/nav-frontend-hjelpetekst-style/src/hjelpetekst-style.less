@import (reference) '~nav-frontend-core/less/_variabler';
@import (reference) '~nav-frontend-core/less/_mixins';
@import (reference) '~nav-frontend-core/less/scaffolding';
@import (reference) '~nav-frontend-lukknapp-style/src/lukknapp-style';
@import (reference) '~nav-frontend-typografi-style/src/index';
@import (reference) '~nav-frontend-lenker-style/src/lenker-mixins';

@desktop: ~'only screen and (min-width: 480px)';

.hjelpetekst {
    position: relative;
    padding-left: 1.25rem;
    display: inline-block;
}

.hjelpetekst .lukknapp {
    .lukknapp();
    position: absolute;
    top: 1rem;
    right: 1rem;
}

.hjelpetekst__tekst:before {
    content: '';
    display: block;
    width: 4rem;
    height: 3rem;
    float: right;
}

.hjelpetekst__apneknapp {
    margin: 0;
    padding: 0;
    border: 0;
    cursor: pointer;
    background-color: transparent;

    &:focus,
    &:active,
    &:hover {
        outline: 0;
        color: @white;
    }

    &:focus .hjelpetekst__anchor {
        outline: 0;
        box-shadow: 0 0 0 3px @fokusFarge;
    }
}

.hjelpetekst__apneknapp.hjelpetekst__apneknapp--med-beskrivelse {
    display: flex;
    padding: 0.2rem 0.5rem;

    &:focus,
    &:active,
    &:hover {
      color: @navBla;
    }

    &:focus {
        outline: 0;
        box-shadow: 0 0 0 3px @fokusFarge;
    }

    &:focus .hjelpetekst__anchor {
        box-shadow: none;
    }

    .hjelpetekst__anchor {
        margin-left: 0.5rem;
    }
}

.hjelpetekst__anchor {
    border-radius: 50%;
    width: 1.5rem;
    height: 1.5rem;
}

.hjelpetekst__container {
    display: inline-block;
    position: relative;
}

.hjelpetekst__tooltip {
    position: fixed;
    top: 0.5rem;
    left: 0.5rem;
    bottom: 0.5rem;
    right: 0.5rem;
    z-index: 1000;
    background-color: @navMorkGra;
    color: @white;
    padding: 1rem;

    &:focus {
      outline: 0;
      box-shadow: 0 0 0 3px @fokusFarge;
    }

    @media @desktop {
        position: absolute;
        width: 24rem;
        top: auto;
        left: auto;
        bottom: auto;
        right: auto;
        border-radius: 0.25rem;

        &:before,
        &:after {
            content: '';
            height: sqrt(2.25rem / 2); // Pythagoras
            width: sqrt(2.25rem / 2); // Pythagoras
            background-color: @navMorkGra;
            display: block;
            position: absolute;
            transform: rotate(-45deg);
            transform-origin: 0 0;
            z-index: 1;
            border: 3px solid transparent;
            box-sizing: border-box;
        }

        &:after {
            z-index: 0;
            background-color: transparent;
            transform: rotate(-45deg);
            box-sizing: content-box;
            border: 3px solid transparent;
        }
    }
}

@media @desktop {
    .hjelpetekst__tooltip--over,
    .hjelpetekst__tooltip--midtstilt {
        left: 0;
        bottom: 3rem;

        &:before,
        &:focus:after {
            top: 100%;
            bottom: 3rem;
            left: 1.25rem;
        }

        &:focus:after {
            left: 1rem;
            border-left-color: @fokusFarge;
            border-bottom-color: @fokusFarge;
        }
    }

    .hjelpetekst__tooltip--midtstilt {
        left: ~'calc(20px + 50%)';
        transform: translateX(~'calc(-50% - 10px)');

        &:before,
        &:focus:after {
            transform-origin: 50% 50%;
            transform: translate(-50%, -50%) rotate(-45deg);
            left: 50%;

        }
    }

    .hjelpetekst__tooltip--under,
    .hjelpetekst__tooltip--under-venstre {
        left: 0;
        top: 3rem;

        &:before,
        &:after {
            top: 0;
        }

        &:focus:after {
            border-top-color: @fokusFarge;
            border-right-color: @fokusFarge;
        }

        &:before {
            left: ~'calc(1rem + 4px)';
        }
    }

    .hjelpetekst__tooltip--under-venstre {
        left: auto;
        right: -1.5rem;

        &:before,
        &:after {
            left: auto;
            right: 1.95rem;
        }

        &:focus:after {
            right: ~'calc(1.95rem - 2px)';
        }
    }

    .hjelpetekst__tooltip--hoyre {
        left: 4rem;
        top: -2rem;
        bottom: auto;

        &:before {
            top: 2rem;
        }

        &:before,
        &:focus:after {
            left: 0;
            transform: rotate(45deg);
        }

        &:focus:after {
            top: 1.75rem;
            left: 0;
            border-left: 3px solid @fokusFarge;
            border-bottom: 3px solid @fokusFarge;
        }
    }

    .hjelpetekst__tooltip--venstre {
        right: 3rem;
        left: auto;
        top: -2rem;
        bottom: auto;

        &:before {
            top: 2rem;
        }

        &:before,
        &:focus:after {
            right: 0;
            transform-origin: 100% 0;
        }

        &:focus:after {
            top: 1.75rem;
            right: 0;
            border-right: 3px solid @fokusFarge;
            border-bottom: 3px solid @fokusFarge;
        }
    }
}
