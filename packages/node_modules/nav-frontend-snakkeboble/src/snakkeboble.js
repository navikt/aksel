import React, { Component, PropTypes as PT } from 'react';
import 'nav-frontend-snakkeboble-style'; // eslint-disable-line import/extensions
import { Panel } from 'nav-frontend-paneler';
import { Undertekst, Normaltekst } from 'nav-frontend-typografi';
import classNames from 'classnames';

const snakkebobleCls = (hoyre) => classNames('snakkeboble', {
    'snakkeboble--hoyre': hoyre, 'snakkeboble--venstre': !hoyre
});

const snakkeboblePilCls = (hoyre) => classNames('snakkeboble__snakkebole-pil', {
    'snakkeboble__snakkebole-pil--hoyre': hoyre,
    'snakkeboble__snakkebole-pil--venstre': !hoyre
});

const snakkebobleIkonCls = (fraVeileder) => classNames('snakkeboble__ikon', {
    'snakkeboble__ikon--veileder': fraVeileder,
    'snakkeboble__ikon--bruker': !fraVeileder
});

const snakkeboblePanelCls = (hoyre) => classNames('snakkeboble__panel snakkeboble-panel', {
    'hoyrejustert-tekst': hoyre
});
/**
 * En snakkeboble for dialog
 */
class Snakkeboble extends Component { // eslint-disable-line react/prefer-stateless-function
    render() {
        const { dato, tekst, pilHoyre } = this.props;
        return (
            <div className="nav-frontend-snakkeboble">
                <div>
                    <div className={snakkebobleCls(pilHoyre)}>
                        <i className={snakkebobleIkonCls(pilHoyre)} />
                        <div className="snakkeboble__snakkeboble-pil-container">
                            <i className={snakkeboblePilCls(pilHoyre)} />
                        </div>
                        <Panel className={snakkeboblePanelCls(pilHoyre)}>
                            <Undertekst className="snakkeboble-panel__dato">{dato}</Undertekst>
                            <Normaltekst className="snakkeboble-panel__tekst">{tekst}</Normaltekst>
                        </Panel>
                    </div>
                </div>
            </div>
        );
    }
}

Snakkeboble.defaultProps = {
    /**
     * Default er at pilen peker mot venstre
     */
    pilHoyre: false
};

Snakkeboble.propTypes = {
    dato: PT.string.isRequired,
    tekst: PT.string.isRequired,
    /**
     * Settes denne peker pilen til snakkeboblen mot høyre, og snakkeboblen og teksten i snakkeboblen flyter mot høyre.
     */
    pilHoyre: PT.bool
};

export default Snakkeboble;
