...
class ToggleGruppe extends React.Component<ToggleGruppeProps, ToggleGruppeState> {
    constructor(props) {
        super(props);
        this.state = {
            pressedIndexes: this.getDefaultPressedToggles()
        };
    }

    getDefaultPressedToggles = () => {
        const array: number[] = [];
        return array.concat(this.props.toggles.filter((toggle) => toggle.pressed).map(
            (pressedToggle) => this.props.toggles.indexOf(pressedToggle)
        ));
    }
