...
const getTextData = () => {
  const textDataContext = require.context(
    "NavFrontendModules",
    true,
    /(\w+)\.(\w+)\.(mdx?)/
  );
...
const getOverviewModulesByPackageName = (packageName, overviewModules) =>
  Object.keys(overviewModules).filter(
    (overviewKey) => overviewKey.indexOf(`./${packageName}/`) !== -1
  );
...
const getComponentData = () => {
  ...
  const pkgContext = require.context(
    "NavFrontendModules",
    true,
    /package\.json/
  );
  ...
  const overviewContext = require.context(
    "NavFrontendModules",
    true,
    /\w+\.overview\.mdx/
  );
  ...
  const allModulesContext = require.context(
    "NavFrontendModules",
    true,
    /lib\/[a-z-]+.js/
  );
  ...
  Object.keys(overviewModules).forEach((overviewKey) => {
    ...
    const pkgOverviewModules = getOverviewModulesByPackageName(
      pkgName,
      overviewModules
    );
    ...
    if (pkgOverviewModules.length > 1) {
      ...
      mainModuleKey = Object.keys(pkgModules).find(
        (moduleKey) =>
      ...
      // All others
      mainModule = pkgModules
        ? allModules[mainModuleKey]["default"]
        : undefined;
    }
...
const components = Object.keys(componentData).map((componentName) => ({
