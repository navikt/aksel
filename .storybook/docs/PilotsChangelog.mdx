...
Updates to Button and Tag components.

Removed classes for defining variants. Variant styling is now done using `data-variant`-attribute on the component.

- As for color-selection, variant is translated to `data-color-role`-attribute on the component for "theming".

```
// Button.tsx | before
<button
  className="navds-button navds-button--primary-neutral"
/>
