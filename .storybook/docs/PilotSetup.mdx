...
For new projects, setup is just as easy as before, but now requires `Theme`-component to be wrapped around your app.
...
- 2. Import `Theme`-component from `@navikt/ds-react`
...
The `Theme`-component has a `theme`-prop that can be set to either `light` or `dark`. This only sets a `class` on the element of `light` or `dark`, and the rest is handled by the CSS.
