...
| 6 | A11y | Checkbox/radio inputs lack visible `aria-label` tied to row content | 🟡 | Selection checkboxes have no accessible name describing *which* row they control. Causes "checkbox" to be announced without context. Use the `label` from the `isRowHeader` column or a `getRowLabel` function. |
