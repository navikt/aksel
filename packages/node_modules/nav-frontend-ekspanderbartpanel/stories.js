import React from 'react';
import { storiesOf } from '@storybook/react';
import { Normaltekst } from 'nav-frontend-typografi';
import { JSDokumentasjon } from './../../dokumentasjon';
import EkspanderbartPanel from './src/index';
import readme from './README.md';
import pkg from './package.json';

storiesOf('Paneler', module)
    .addWithSections('Ekspanderbart panel', (_, action) => (
        <div style={{ padding: '1rem', backgroundColor: '#e9e7e7' }}>
            <EkspanderbartPanel tittel="Test" tittelProps="innholdstittel" onClick={action('ekspander')}>
                <Normaltekst>Dette er ett expandpanel</Normaltekst>
            </EkspanderbartPanel>
            <EkspanderbartPanel tittel="Test" onClick={action('ekspander')}>
                <Normaltekst>Dette er ett expandpanel</Normaltekst>
            </EkspanderbartPanel>
            <EkspanderbartPanel tittel="Test" tittelProps="systemtittel" onClick={action('ekspander')}>
                <Normaltekst>Dette er ett expandpanel</Normaltekst>
            </EkspanderbartPanel>
            <EkspanderbartPanel tittel="Test" tittelProps="element" onClick={action('ekspander')}>
                <Normaltekst>Dette er ett expandpanel med element header</Normaltekst>
            </EkspanderbartPanel>
        </div>
    ), JSDokumentasjon(pkg, readme, EkspanderbartPanel))
    .addWithSections('Nøstede paneler', (_, action) => (
        <div style={{ padding: '1rem', backgroundColor: '#e9e7e7' }}>
            <EkspanderbartPanel tittel="Test" tittelProps="innholdstittel" onClick={action('ekspander')}>
                <Normaltekst>Dette er ett expandpanel</Normaltekst>
                <EkspanderbartPanel tittel="Nøstet Test" tittelProps="innholdstittel" onClick={action('ekspander')}>
                    <Normaltekst>Dette er ett expandpanel</Normaltekst>
                </EkspanderbartPanel>
            </EkspanderbartPanel>
        </div>
    ))
    .add('Fokus', () => (
        <div style={{ padding: '1rem', backgroundColor: '#e9e7e7' }}>
            <EkspanderbartPanel tittel="Panel 1">
                <h1>Innhold 1</h1>
                <button>Kan få fokus</button>
            </EkspanderbartPanel>
            <EkspanderbartPanel tittel="Panel 2">
                <h1>Innhold 2</h1>
                <button>Kan få fokus</button>
            </EkspanderbartPanel>
        </div>
    ));
