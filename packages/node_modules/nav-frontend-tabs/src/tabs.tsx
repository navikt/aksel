...
export interface TabsProps extends TabsPureProps {
    ...
    defaultAktiv?: number;
    /**
     * Wrapper innhold i Panel, default false
     */
    panel?: boolean;
}
...
class Tabs extends React.Component<TabsProps, TabsState> {
    ...
    wrapIntoPanel(isPanel, children) {
        return isPanel ? <Panel className="panel--tab"> {children} </Panel> : children;
    }

    renderContent() {
        return React.Children.map(this.props.children, (child: any, i) => {
            const wrapped = this.wrapIntoPanel(this.props.panel, child.props.children);
            return (child.type.name === Tab.name && i === this.state.aktivTab) && wrapped;
        });
    }
