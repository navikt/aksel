...
export function getScrollParents(element) {
    ...
    for (let parent = element; (parent = parent.parentElement); parent !== document.body) {
        style = getComputedStyle(parent);
        if (excludeStaticParent && style.position === "static") {
            continue;
        }
        if (overflowRegex.test(style.overflow + style.overflowY + style.overflowX)) scrollParents.push(parent);
    }
