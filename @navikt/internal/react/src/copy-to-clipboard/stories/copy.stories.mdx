import { Meta, Canvas } from "@storybook/addon-docs";
import { CopyToClipboard } from "../../index";
import { Success } from "@navikt/ds-react";

<Meta title="ds-react-internal/copy-to-clipboard/intro" />

# Hvordan ta i bruk CopyToClipboard

Innholdet i `copyText` vil bli kopiert til clipboard, mens innholdet i `popoverText`
brukes i popover. Hvis komponenten ikke har noen children (tekst i button) vil da en "navds-sr-only" tekst bli
lagt på knappen med tekst "Kopier copyText"

```jsx
<CopyToClipboard popoverText={"Kopierte adresse"} copyText={"0660 OSLO"}>
  Kopier adresse
</CopyToClipboard>
```

<Canvas>
  <CopyToClipboard popoverText={"Kopiert adresse"} copyText={"0660 OSLO"}>
    Kopier adresse
  </CopyToClipboard>
</Canvas>

## Sizing

```jsx
<CopyToClipboard
  popoverText={"Kopierte adressen"}
  copyText={"0660 OSLO"}
  size="small"
>
  Kopier adresse
</CopyToClipboard>
```

<Canvas>
  <CopyToClipboard
    popoverText={"Kopierte adressen"}
    copyText={"0660 OSLO"}
    size="small"
  >
    Kopier adresse
  </CopyToClipboard>
</Canvas>
