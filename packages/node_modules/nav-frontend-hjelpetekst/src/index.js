...
class HjelpetekstBase extends Component {
    ...
    renderContent(props) {
        return (
            <span className={cls(props.type, classNames)}>{props.children}
                <a className="hjelpetekst__stangknapp" onClick={this.toggleSynlighet}></a>
            </span>
        );
    }
    render(props) {
        const Hjelpetekst = this.state.erSynlig ? this.renderContent(this.props) : null;
        return (
