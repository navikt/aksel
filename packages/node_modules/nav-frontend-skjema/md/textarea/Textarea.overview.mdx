import Example from './../../../../../guideline-app/app/ui/components/example/Example';
import { Textarea, TextareaControlled } from './../../';
import TextareaExample from './../../sample/_textarea.example.js';
import Alertstripe from 'nav-frontend-alertstriper';

## Normal

<Example>
    <TextareaControlled label="Textarea-label" />
</Example>

```jsx
<TextareaControlled label="Textarea-label" />
```

## Disabled

<Example>
    <TextareaControlled label="Textarea-label" disabled />
</Example>

```jsx
<TextareaControlled label="Textarea-label" disabled />
```

## Feilmelding

<Example>
    <TextareaControlled label="Textarea-label" feil={{feilmelding: 'Her er det noe feil.'}} />
</Example>

```jsx
<TextareaControlled label="Textarea-label" disabled />
```

## Manuell state-håndtering

Hvis du ønsker å håndtere state manuelt så kan du bruke `Textarea` i stedet for `TextareaControlled`

<Example>
    <TextareaExample />
</Example>

```jsx
<Textarea 
    value={this.state.value}
    onChange={(e) => this.setState({ value: e.target.value })}
/>
```