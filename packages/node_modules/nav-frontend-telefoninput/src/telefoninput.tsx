...
import DropdownList from 'nav-frontend-telefoninput/src/components/dropdownliste';
import countryData from './telefoninput-data.js';
import { TelefonInputProps, Country } from 'nav-frontend-telefoninput/src/telefoninput-types';
...
class TelefonInput extends React.Component<TelefonInputProps, State> {
    ...
    constructor(props) {
        ...
        if (regions) {
            filteredCountries = this.filterRegions(regions, filteredCountries);
        }

        if (Object.keys(masks).length !== 0) {
            filteredCountries = this.insertMasks(masks, filteredCountries);
        }
        ...
        const preferredCountries = filteredCountries.filter((country) =>
             props.preferredCountries.some((preferredCountry) => preferredCountry === country.iso2)
        );
    ...
    handleInput = (e) => {
        ...
        if (value.replace(/\D/g, '').length - selectedCountry.dialCode.length > 16) {
            return;
        }
    ...
    lokalFeil = (inputNumber: string) => {
        ...
        if (!startsWithCountryCode) {
            return {
                feilmelding: 'Telefonnummer må starte med en gyldig landkode'
            };
        }
        ...
        if (!harNoksiffer) {
            return {
                feilmelding: 'Norske telefonnummer må ha 8 siffer'
            };
        }
        ...
        return (
            <div className="react-tel-input">
                ...
                >
                    ...
                    {showDropdown &&
                        <DropdownList
                            enableSearchField={this.props.enableSearchField}
                            searchPlaceholder={this.props.searchPlaceholder}
                            highlightCountryIndex={this.state.highlightCountryIndex}
                            preferredCountries={this.state.preferredCountries}
                            showDropdown={this.state.showDropdown}
                            searchValue={this.state.searchValue}
                            selectedCountry={this.state.selectedCountry}
                            filterCountriesBySearch={this.filterCountriesBySearch}
                            handleFlagItemClick={this.handleFlagItemClick}
                            getDropdownCountryName={this.getDropdownCountryName}
                            handleSearchChange={this.handleSearchChange}
                            handleKeydown={this.handleKeydown}
                            setDropdownRef={this.setDropdownRef}
                            setFlagRef={this.setFlagRef}
                        />
                    }
                </div>
            </div>
