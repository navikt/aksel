    ...
    >
      ...
      {isDraftMode ? (
        <Suspense fallback={null}>
          <DynamicFrontpageContent />
        </Suspense>
      ) : (
        <CachedFrontpageContent perspective="published" stega={false} />
      )}
    </AkselPage>
  );
};

async function DynamicFrontpageContent() {
  const { perspective, stega } = await getDynamicFetchOptions();
  return <CachedFrontpageContent perspective={perspective} stega={stega} />;
}

async function CachedFrontpageContent({
  perspective,
  stega,
}: DynamicFetchOptions) {
  "use cache";
