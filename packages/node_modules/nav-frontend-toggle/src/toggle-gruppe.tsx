...
class ToggleGruppe extends React.Component<ToggleGruppeProps, ToggleGruppeState> {
    constructor(props) {
        super(props);
        this.state = {
            active: this.props.defaultToggles.map(({ pressed }, index, array) => pressed
             || (!!this.props.minstEn && index === 0 && array.every((toggle) => !toggle.pressed)))
        };
    }
    ...
    handleClick = (e, index, customOnClick) => {
        ...
        const newActive = this.state.active.map((active, i, array) => {
            if (this.props.multiSelect) {
                if (this.props.minstEn && array.filter(Boolean).length === 1 && array.indexOf(true) === index) {
                    return active;
                }
        ...
        if (typeof this.props.onChange === 'function') {
            this.props.onChange(e, this.props.defaultToggles.map((props, i) => ({ ...props, pressed: newActive[i] })));
        }
    }
